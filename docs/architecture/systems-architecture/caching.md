...
1. Где в цепочке запросов кэш даст максимальный эффект?
2. Для каких данных допустима eventual consistency?
3. Как обрабатывается stampede при массовом истечении TTL?
4. Какие метрики показывают реальную эффективность кэша?

## Чек-лист самопроверки

- выбран тип кэша и паттерн по сценарию;
- настроены TTL и invalidation;
- реализована защита от stampede;
- есть hit/miss/latency метрики;
- задокументирована политика security/PII в кэше.
