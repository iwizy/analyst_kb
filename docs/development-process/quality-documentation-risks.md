# Качество, документация и управление рисками

Раздел описывает минимальный управленческий контур, без которого процесс разработки теряет предсказуемость и устойчивость.

## Уровни сложности

### Базовый уровень

- понимать уровни тестирования и роль quality gates;
- вести обязательную проектную документацию;
- фиксировать и отслеживать ключевые риски.

### Средний уровень

- строить живую документацию (docs-as-code);
- внедрять risk matrix и план реагирования;
- настраивать контроль качества на каждом этапе SDLC.

### Продвинутый уровень

- связывать качество, документацию и риски с DORA/SLO-метриками;
- автоматизировать проверки качества и compliance;
- проводить процессные улучшения на основе postmortem и RCA.

## Управление качеством

| Направление | Практики | Артефакты |
| --- | --- | --- |
| Функциональное качество | unit/integration/system/UAT | test plan, test report |
| Техническое качество | code review, static analysis, architecture review | review log, tech debt register |
| Производительность | load/stress/soak tests | perf report, capacity recommendations |
| Безопасность | SAST/DAST, dependency scan, pentest | security findings backlog |

### Quality gates перед релизом

- выполнены AC и DoD;
- критичные дефекты закрыты или согласован workaround;
- пройдены обязательные security/performance проверки;
- актуализированы runbook и rollback plan;
- подтверждены операционные сценарии (мониторинг, алерты, эскалация).

## Документация

### Подходы

- docs-as-code (Markdown + Git + review);
- wiki/Confluence для процессных и организационных знаний;
- ADR для архитектурных и процессных решений;
- release notes и change log для эксплуатационного контура.

### Минимальный набор документов

| Артефакт | Назначение | Владелец |
| --- | --- | --- |
| BRD/SRS или backlog-спецификация | формализует требования | BA/SA + PO |
| ADR | фиксирует архитектурные решения | архитектор + SA |
| Test plan/report | подтверждает качество | QA |
| Runbook | описывает эксплуатационные действия | DevOps/SRE |
| Risk register | управляет неопределенностью | PM/SA |

### Шаблон ADR

| Поле | Содержание |
| --- | --- |
| ID | ADR-2026-02-010 |
| Контекст | проблема и ограничения |
| Варианты | рассмотренные альтернативы |
| Решение | выбранный вариант |
| Последствия | плюсы/минусы/риски |
| План пересмотра | когда и как ревизировать |

## Управление рисками

### Процесс

1. Идентификация рисков.
2. Классификация (технические, бизнес, организационные, внешние).
3. Оценка: вероятность x влияние.
4. План реагирования: avoid, mitigate, transfer, accept.
5. Мониторинг и отчетность.

### Матрица риска (пример)

| Риск | Вероятность | Влияние | Приоритет | Мера |
| --- | --- | --- | --- | --- |
| Задержка внешнего API | Высокая | Высокое | Критичный | контрактные тесты + fallback |
| Потеря ключевого эксперта | Средняя | Высокое | Высокий | knowledge transfer + backup owner |
| Рост облачных затрат | Средняя | Среднее | Средний | FinOps-контроль и бюджетные лимиты |

## RACI по документации и рискам

| Активность | BA/SA | PO | Архитектор | QA | DevOps/SRE | PM |
| --- | --- | --- | --- | --- | --- | --- |
| Актуализация требований | R | A | C | C | I | C |
| Ведение ADR | C | I | A/R | I | C | I |
| Обновление runbook | C | I | C | C | A/R | I |
| Ведение risk register | R | C | C | C | C | A |
| Релизный риск-review | C | A | C | C | C | R |

## Чек-листы по фазам SDLC

### Анализ и дизайн

- требования измеримы и трассируемы;
- архитектурные решения задокументированы;
- риски и зависимости формализованы.

### Разработка и тестирование

- автоматические проверки встроены в pipeline;
- coverage и quality thresholds соблюдены;
- дефекты классифицированы и приоритизированы.

### Релиз и эксплуатация

- runbook и план rollback проверены;
- SLA/SLO и alerting согласованы;
- post-release review запланирован.

## Схема контроля качества и рисков

```kroki-plantuml
skinparam monochrome true
start
:Сформировать требования;
:Проверить quality criteria;
:Оценить риски и зависимости;
if (Quality/Risk gates пройдены?) then (Да)
  :Разработка и тестирование;
  :Релиз;
else (Нет)
  :Корректировки и mitigation plan;
endif
:Эксплуатационный контроль и feedback;
stop
```

## Контрольные вопросы

1. Какие quality gates блокируют выпуск, а какие носят уведомительный характер?
2. Где хранится актуальная версия обязательной документации?
3. Как риск-реестр связан с backlog и релизным планом?
4. Какие метрики подтверждают улучшение качества процесса?

## Чек-лист самопроверки

- процесс качества описан и повторяем;
- документация живая и привязана к релизному циклу;
- риски оцениваются и пересматриваются регулярно;
- решения по качеству и рискам имеют владельцев;
- есть прозрачная отчетность по статусу рисков и дефектов.

## Стандарты и источники

- ISO/IEC/IEEE 12207: <https://www.iso.org/standard/63712.html>
- ISO 31000 Risk Management: <https://www.iso.org/iso-31000-risk-management.html>
- NIST SSDF: <https://csrc.nist.gov/Projects/ssdf>
- DORA research: <https://dora.dev/research/>
