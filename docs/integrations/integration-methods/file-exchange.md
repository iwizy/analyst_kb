...
Форматы файлов Чаще всего используются форматы:
...
Передача файлов Основные способы передачи:
...
Периодичность передачи Интеграции могут быть:
...
Обработка файлов Получающая сторона:
...
Создание файла заказа (на стороне OMS) OMS генерирует CSV-файл с данными о заказах:

```
order_id,product_id,quantity,delivery_date 101,501,3,2025-01-28 102,502,1,2025-01-29 103,503,5,2025-01-30
```

Передача файла Файл передается на сервер WMS через SFTP:

- Адрес: sftp.warehouse.com
- Логин: oms_user
- Папка: /incoming_orders/
Обработка файла (на стороне WMS) WMS обрабатывает файл:
...
Ответное уведомление (на стороне WMS) После обработки WMS создает файл ответа:

```
order_id,status,comment 101,accepted, 102,accepted, 103,rejected,Product out of stock
```
...
> Материал адаптирован по статье `https://iwizy.github.io/integrations/types/files`.
