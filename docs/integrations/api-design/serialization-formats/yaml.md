...
YAML чаще используется для конфигураций, IaC и pipeline-описаний, но реже как формат бизнес-payload в публичных API.
...
| CI/CD pipeline | основной формат в GitOps-практиках |
| Kubernetes манифесты | стандарт де-факто |
| Бизнес payload API | обычно лучше JSON/XML |
...
1. Где YAML оправдан, а где лучше JSON/XML?
2. Какая схема валидации применяется для YAML-файлов?
3. Как исключены ошибки отступов и неявных типов?
4. Есть ли контроль изменений конфигураций в CI?

## Чек-лист самопроверки

- YAML используется в подходящих сценариях;
- внедрены lint и schema validation;
- есть style guide и правила ревью;
- конфигурации версионируются и трассируются;
- rollback конфигураций отработан.
