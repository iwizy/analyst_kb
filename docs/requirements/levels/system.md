...
- зафиксировать FR/[NFR](../../glossary.md#abbr-053), контракты и ограничения;
- создать основу для архитектуры, разработки и [QA](../../glossary.md#abbr-067).

## Состав системных требований

- функциональные требования (FR);
- нефункциональные требования (NFR);
- интерфейсные требования ([API](../../glossary.md#abbr-005), события, форматы данных);
...
1. Добавить [NFR](../../glossary.md#abbr-053) для критичных потоков.
1. Уточнить [API](../../glossary.md#abbr-005)/данные/интеграции и контрактные ограничения.
...
## Пример SR + [API](../../glossary.md#abbr-005) контракт

| Поле | Пример |
| --- | --- |
| SR ID | SR-RET-022 |
| [Контракт](../../glossary.md#term-038) | `POST /refunds/calculate` |
| Вход | `orderId`, `positions[]`, `reasonCode` |
| Выход | `refundAmount`, `currency`, `rules[]` |
| Ошибка | `422 INVALID_REASON`, `503 BILLING_UNAVAILABLE` |
| [NFR](../../glossary.md#abbr-053) | p95 <= 1 сек при 100 [RPS](../../glossary.md#abbr-075) |

## Примеры SR по доменам

| [Домен](../../glossary.md#term-023) | Пример SR | Проверка |
| --- | --- | --- |
| Финтех | [Сервис](../../glossary.md#term-060) платежей выполняет retry c экспоненциальной задержкой и идемпотентностью по `paymentId` | Контрактные тесты + отказоустойчивый стенд |
| E-commerce | Модуль возвратов рассчитывает компенсацию с учетом скидок и комиссий | Functional + performance test |
| Госуслуги | Шлюз межведомственного обмена обрабатывает входящие статусы не дольше 10 минут | Интеграционный [SLA](../../glossary.md#abbr-079)-тест |
...
- формируются заранее в составе [SRS](../../glossary.md#abbr-085);
...
- проводить совместный review BA + Tech Lead + [QA](../../glossary.md#abbr-067).
