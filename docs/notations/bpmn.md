...
- связывать BPMN с требованиями и SLA.
...
- BPMN + C4: C4 показывает архитектурный контекст, BPMN — flow операций внутри контекста.
- BPMN + UML Sequence: Sequence деталирует взаимодействие систем в одном шаге BPMN.
- BPMN + ERD: ERD уточняет данные, которые процесс создает и изменяет.
...
1. Явно ли обозначены start/end и все ключевые исключения?
2. Видно ли, кто отвечает за каждый шаг?
3. Есть ли на схеме понятные условия ветвления?
4. Возможно ли автоматизировать процесс по этой модели?

## Чек-лист самопроверки

- процесс ограничен понятным scope;
- роли и потоки сообщений обозначены;
- задачи названы действиями;
- исключения и таймеры учтены;
