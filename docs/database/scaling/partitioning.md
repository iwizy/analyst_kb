...
- Увеличить производительность запросов за счет работы только с нужными партициями.
- Упростить управление данными, например, удаление устаревших данных.
- Снизить нагрузку на сервер.
- Ускорить операции ввода/вывода, такие как вставка, обновление и удаление.
- Обеспечить гибкость резервного копирования и восстановления данных.
...
Пример: Вы хотите разделить таблицу заказов по годам. Это позволяет хранить заказы за каждый год в отдельной партиции.

```
CREATE TABLE orders ( order_id INT, order_date DATE, amount DECIMAL(10, 2) ) PARTITION BY RANGE (YEAR(order_date)) ( PARTITION p_2021 VALUES LESS THAN (2022), -- Данные за 2021 год PARTITION p_2022 VALUES LESS THAN (2023), -- Данные за 2022 год PARTITION p_max VALUES LESS THAN MAXVALUE -- Данные за последующие годы );
```
...
Пример: Разделим таблицу клиентов на партиции в зависимости от их региона.

```
CREATE TABLE customers ( customer_id INT, region VARCHAR(50) ) PARTITION BY LIST (region) ( PARTITION p_north VALUES IN ('North', 'Northeast'), PARTITION p_south VALUES IN ('South', 'Southeast'), PARTITION p_west VALUES IN ('West', 'Southwest'), PARTITION p_other VALUES IN ('Central', 'Unknown') );
```
...
Пример: Разделим таблицу логов на 4 партиции, используя идентификатор лога.

```
CREATE TABLE logs ( log_id INT, log_message TEXT ) PARTITION BY HASH (log_id) PARTITIONS 4;
```
...
Пример: Разделим таблицу продаж сначала по годам, а затем разбиваем каждую годовую партицию по регионам.

```
CREATE TABLE sales ( sale_id INT, sale_date DATE, region VARCHAR(50) ) PARTITION BY RANGE (YEAR(sale_date)) SUBPARTITION BY HASH (region) SUBPARTITIONS 4 ( PARTITION p_2021 VALUES LESS THAN (2022), PARTITION p_2022 VALUES LESS THAN (2023) );
```
...
- Цели оптимизации производительности.
Выбор правильной стратегии партицирования позволяет ускорить работу системы и упростить управление данными.

> Материал адаптирован по статье `https://iwizy.github.io/database/scaling/partitioning`.
