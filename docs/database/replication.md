...
- Клиент отправляет запрос на запись в Master.
- Master передаёт данные на все реплики.
- Реплики подтверждают успешную запись.
- Только после этого клиент получает подтверждение о завершении транзакции.
...
- Клиент отправляет запрос на запись в Master.
- Master подтверждает завершение транзакции.
- После подтверждения данные отправляются на реплики в фоновом режиме.
Пример использования:
...
- Все операции записи выполняются на Master.
- Реплики получают данные через механизм репликации и обслуживают запросы на чтение.
Пример использования:
...
- Серверы Master обмениваются данными между собой.
- Конфликты записей разрешаются с помощью пользовательских правил или алгоритмов.
Пример использования:
...
- Настройка Slave:
```
[mysqld] server-id = 2 replicate-do-db = example_db
```
...
```
rs.initiate({ _id: "rs0", members: [ { _id: 0, host: "server1" }, { _id: 1, host: "server2" }, { _id: 2, host: "server3" } ] });
```
...
> Материал адаптирован по статье `https://iwizy.github.io/database/replication`.
